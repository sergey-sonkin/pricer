...
class eBayAPIResearcher:
    ...
    def research_product(self, product_description: str, category_id: str | None = None) -> eBayPriceAnalysis:
        """
    ...
    def _get_sold_listings(self, keywords: str, category_id: str | None = None, max_results: int = 50) -> List[eBayListing]:
        ...
        try:
            ...
            payload = {
                "findCompletedItemsRequest": {
                    ...
                    "paginationInput": {
                        "entriesPerPage": min(max_results, 100)
                    }
                }
            }
            ...
            if data.get('findCompletedItemsResponse', [{}])[0].get('ack', [''])[0] != 'Success':
                error_msg = data.get('findCompletedItemsResponse', [{}])[0].get('errorMessage', {})
    ...
    def _get_active_listings(self, keywords: str, category_id: str | None = None, max_results: int = 25) -> List[eBayListing]:
        ...
        try:
            ...
            # Check for API errors
            if data.get('findItemsByKeywordsResponse', [{}])[0].get('ack', [''])[0] != 'Success':
                error_msg = data.get('findItemsByKeywordsResponse', [{}])[0].get('errorMessage', {})
                print(f"    ❌ eBay API error: {error_msg}")
                return listings
    ...
    def _analyze_ebay_data(self, search_terms: str, sold_listings: List[eBayListing], 
                          active_listings: List[eBayListing]) -> eBayPriceAnalysis:
        ...
        active_prices = [listing.price for listing in active_listings if listing.price > 0]
        ...
        if sold_prices:
            ...
            price_stats = {
                'sold_min': 0, 'sold_max': 0, 'sold_avg': 0, 
                'sold_median': 0, 'sold_count': 0
            }
        ...
        if active_prices:
            ...
            price_stats.update({
                'active_min': 0, 'active_max': 0, 'active_avg': 0,
                'active_median': 0, 'active_count': 0
            })
        
        # Generate market insights
        insights = self._generate_market_insights(price_stats, sold_listings, active_listings)
        
        # Calculate confidence score
        confidence = self._calculate_confidence(len(sold_listings), len(active_listings))
    ...
    def _generate_market_insights(self, stats: Dict[str, float], 
                                 sold_listings: List[eBayListing], 
                                 active_listings: List[eBayListing]) -> List[str]:
        ...
        if stats['sold_count'] > 0:
            ...
            if stats['active_count'] > 0:
                ...
                if stats['active_avg'] > stats['sold_avg'] * 1.2:
                    insights.append("Current listings are priced high compared to recent sales")
                elif stats['active_avg'] < stats['sold_avg'] * 0.8:
                    insights.append("Current market may be soft - competitive pricing opportunity")
                else:
        ...
        auction_count = len([l for l in sold_listings if 'auction' in l.listing_type.lower()])
        if auction_count > 0 and len(sold_listings) > 0:
            auction_pct = (auction_count / len(sold_listings)) * 100
            if auction_pct > 30:
                insights.append(f"{auction_pct:.0f}% of sales were auctions - consider auction format")
    ...
    def print_analysis(self, analysis: eBayPriceAnalysis):
        ...
        if stats['active_count'] > 0:
            ...
            print(f"   • Range: ${stats['active_min']:.2f} - ${stats['active_max']:.2f}")
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python ebay_api_researcher.py <product_description> [category_id]")

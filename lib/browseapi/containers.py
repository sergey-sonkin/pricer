    ...
    def __init__(self, response: dict, method: str, pass_errors: bool):
        ...
        if method in ("search", "search_by_image"):
            ...
            if "itemSummaries" in response:
                self.itemSummaries = [
                    ItemSummary(item) for item in response["itemSummaries"]
                ]
        ...
        # item methods
            ...
            if "additionalImages" in response:
                self.additionalImages = [
                    Image(image) for image in response["additionalImages"]
                ]
            ...
            if "localizedAspects" in response:
                self.localizedAspects = [
                    TypedNameValue(aspect) for aspect in response["localizedAspects"]
                ]
            ...
            if "primaryProductReviewRating" in response:
                self.primaryProductReviewRating = ReviewRating(
                    response["primaryProductReviewRating"]
                )
            ...
            if "shippingOptions" in response:
                self.shippingOptions = [
                    ShippingOption(option) for option in response["shippingOptions"]
                ]
            ...
            if "commonDescriptions" in response:
                self.commonDescriptions = [
                    CommonDescriptions(description)
    ...
    def parse_errors(self, response: dict, pass_errors: bool) -> None:
        ...
        for error in response["errors"]:
            if error["errorId"] in (11000, 12000):
                ...
                11001 <= error["errorId"] <= 11507
                or 12001 <= error["errorId"] <= 12007
                ...
                exception = exceptions.BrowseAPIError(
                    "Unhandled error, code: {}, message: {}".format(
...
class Item(BrowseAPIBaseContainer):
    ...
    def __init__(self, item: dict):
        ...
        if "localizedAspects" in item:
            self.localizedAspects = [
                TypedNameValue(aspect) for aspect in item["localizedAspects"]
            ]
        ...
        if "primaryProductReviewRating" in item:
            self.primaryProductReviewRating = ReviewRating(
                item["primaryProductReviewRating"]
            )
        ...
        if "shippingOptions" in item:
            self.shippingOptions = [
                ShippingOption(option) for option in item["shippingOptions"]
            ]
...
class ItemSummary(BrowseAPIBaseContainer):
    ...
    def __init__(self, item_summary: dict):
        ...
        if "additionalImages" in item_summary:
            self.additionalImages = [
                Image(image) for image in item_summary["additionalImages"]
            ]

        if "categories" in item_summary:
            self.categories = [
                Category(category) for category in item_summary["categories"]
            ]

        if "compatibilityProperties" in item_summary:
            self.categories = [
                CompatibilityProperty(compatibility_property)
        ...
        if "distanceFromPickupLocation" in item_summary:
            self.distanceFromPickupLocation = TargetLocation(
                item_summary["distanceFromPickupLocation"]
            )
        ...
        if "pickupOptions" in item_summary:
            self.pickupOptions = [
                PickupOptionSummary(option) for option in item_summary["pickupOptions"]
            ]

        if "shippingOptions" in item_summary:
            self.shippingOptions = [
                ShippingOptionSummary(option)
                for option in item_summary["shippingOptions"]
            ]

        if "thumbnailImages" in item_summary:
            self.thumbnailImages = [
                Image(image) for image in item_summary["thumbnailImages"]
            ]
...
class SellerLegalInfo(BrowseAPIBaseContainer):
    ...
    def __init__(self, info: dict):
        ...
        if "sellerProvidedLegalAddress" in info:
            self.sellerProvidedLegalAddress = Address(
                info["sellerProvidedLegalAddress"]
            )
...
class ShippingOption(ShippingOptionSummary):
    ...
    def __init__(self, option: dict):
        ...
        if "additionalShippingCostPerUnit" in option:
            self.additionalShippingCostPerUnit = ConvertedAmount(
                option["additionalShippingCostPerUnit"]
            )
        ...
        if "shipToLocationUsedForEstimate" in option:
            self.shipToLocationUsedForEstimate = ShipToLocation(
                option["shipToLocationUsedForEstimate"]
            )
...
class ShipToLocations(BrowseAPIBaseContainer):
    ...
    def __init__(self, locations: dict):
        if "regionExcluded" in locations:
            self.regionExcluded = [
                Region(region) for region in locations["regionExcluded"]
            ]

        if "regionIncluded" in locations:
            self.regionIncluded = [
                Region(region) for region in locations["regionIncluded"]
            ]
...
class AspectDistribution(BrowseAPIBaseContainer):
    ...
    def __init__(self, distribution):
        ...
        if "aspectValueDistributions" in distribution:
            self.aspectValueDistributions = [
                AspectValueDistribution(value_distribution)
...
class ItemGroupSummary(BrowseAPIBaseContainer):
    ...
    def __init__(self, summary: dict):
        ...
        if "itemGroupAdditionalImages" in summary:
            self.itemGroupAdditionalImages = [
                Image(image) for image in summary["itemGroupAdditionalImages"]
            ]
...
class ReviewRating(BrowseAPIBaseContainer):
    ...
    def __init__(self, review: dict):
        ...
        if "ratingHistograms" in review:
            self.ratingHistograms = [
                RatingHistogram(histogram) for histogram in review["ratingHistograms"]
            ]
...
class Product(BrowseAPIBaseContainer):
    ...
    def __init__(self, product: dict):
        ...
        if "additionalImages" in product:
            self.additionalImages = [
                Image(image) for image in product["additionalImages"]
            ]

        if "additionalProductIdentities" in product:
            self.additionalProductIdentities = [
                AdditionalProductIdentity(identity)
                for identity in product["additionalProductIdentities"]
            ]

        if "aspectGroups" in product:
            self.aspectGroups = [
                AspectGroup(aspect_group) for aspect_group in product["aspectGroups"]
            ]
...
class ErrorDetailV3(BrowseAPIBaseContainer):
    ...
    def __init__(self, warning: dict):
        ...
        if "parameters" in warning:
            self.parameters = [
                ErrorParameterV3(parameter) for parameter in warning["parameters"]
            ]

...
class ClientTest(TestCase):
    ...
    def test_init_params(self):
        self.assertRaises(
            BrowseAPIParamError, BrowseAPI, "app_id", "cert_id", reference_id="ref_id"
        )
        self.assertRaises(
            BrowseAPIParamError, BrowseAPI, "app_id", "cert_id", country="US"
        )
        self.assertRaises(
            BrowseAPIParamError, BrowseAPI, "app_id", "cert_id", zip_code=19026
        )

    def test_search(self):
        responses = self.api.execute(
            "search", [{"q": "drone", "limit": 50}, {"category_ids": 20863}]
        )
    ...
    def test_get_item(self):
        responses = self.api.execute(
            "get_item", [{"item_id": "v1|202117468662|0", "fieldgroups": "PRODUCT"}]
        )
        self.assert_responses(responses)
        responses = self.api.execute(
            "get_item", [{"item_id": "v1|202117468662|0", "fieldgroups": "COMPACT"}]
        )
        self.assert_responses(responses)

    def test_get_item_by_legacy_id(self):
        responses = self.api.execute(
            "get_item_by_legacy_id", [{"legacy_item_id": "262800155662"}]
        )
        self.assert_responses(responses)
    ...
    def test_get_items_by_item_group(self):
        responses = self.api.execute(
            "get_items_by_item_group", [{"item_group_id": "351825690866"}]
        )
        self.assert_responses(responses)
    ...
    def test_check_compatibility(self):
        responses = self.api.execute(
            ...
            [
                {
                    "item_id": "v1|182708228929|0",
                    "compatibility_properties": self.properties,
                }
            ],
        )
